...
function checker() {
    ...
    console.log('Is it a leap year?', leapyear);
    ...
    }if(months!== 2 && months % 2 === 0 && months !== 8){
        dayonmonth = 30;
    }else if(months !== 2 && months % 2 !=0 || months === 8){
        dayonmonth = 31;
    }
    console.log("months:",months);
    console.log(dayonmonth);
    labels.forEach((label, index) => {
        let birthdate = new Date(`${months}/${days}/${years}`);
        let currentdate = new Date();
        if (days === "" && months === "" && years === "") {
            label.classList.add("labelerr");
            errorAlerts[index].innerText = "This field is requiered";
            successElements[index].classList.toggle("success");
        }else if(birthdate > currentdate){
            label.classList.add("labelerr");
            errorAlerts[index].innerText = "Must be valid date";
            successElements[index].classList.toggle("success");
        }else if(days > 31){
            label.classList.add("labelerr");
            errorAlerts[0].innerText = "Must be valid date";
            successElements[index].classList.toggle("success");
            ...
            label.classList.add("labelerr");
            errorAlerts[1].innerText = "Must be valid date";
            successElements[index].classList.toggle("success");
        }else if(years > new Date().getFullYear()){
            label.classList.add("labelerr");
            errorAlerts[2].innerText = "Must be valid date";
            successElements[index].classList.toggle("success");
